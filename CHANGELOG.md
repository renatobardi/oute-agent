...
- Roteamento em 2 etapas: Jev escolhe o **perfil**; o OpenRouter escolhe o **modelo** dentro dele (`models` = todos os modelos elegíveis do perfil, `provider.sort` por perfil, `partition: none`, fallback automático). Perfis também podem ser pedidos direto (`/model coder` no Pi).
